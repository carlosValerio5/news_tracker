...
def aws_helper(mock_sqs_resource, mock_sqs_client, mock_queue):
    ...
    helper = AwsHelper(
        queue_url="https://queue.amazonaws.com/123/test-queue",
        fallback_queue_url="https://queue.amazonaws.com/123/fb",
    )
...
def test_send_batch_multiple_batches(aws_helper):
    ...
    batch_sizes = [
        call.kwargs["Entries"] for call in aws._SQS.send_message_batch.call_args_list
    ]
...
def test_send_to_fallback_no_url(aws_helper, capsys):
    ...
    with pytest.raises(
        Exception, match="Fallback queue not provided for this instance."
    ):
        aws_helper.send_messages_to_fallback_queue(
            [{"Body": "test", "ReceiptHandle": "Test"}]
        )
