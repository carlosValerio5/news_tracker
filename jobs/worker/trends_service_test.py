...
def test_estimate_popularity_sets_has_data_false_when_both_minus_one(service):
    ...
    with (
        ...
        patch.object(
            service,
            "get_start_end_date",
            return_value=(date(2025, 1, 1), date(2025, 1, 7)),
        ),
    ):
        results = service.estimate_popularity("Apple")
...
def test_get_peak_interest_no_timeline_data_logs(service, caplog):
    payload = {"interest_over_time": {"dummy": {}}}
    with caplog.at_level("WARNING"):
        result = service.get_peak_interest(payload)
    assert result == -1
...
def test_get_peak_interest_entry_missing_values_logs(service, caplog):
    payload = {
        "interest_over_time": {"timeline_data": [{"date": "Feb 1 – Feb 7, 2025"}]}
    }
    with caplog.at_level("WARNING"):
        result = service.get_peak_interest(payload)
    assert result == -1
...
def test_get_current_interest_success(service):
    ...
    date_str = (
        f"{yesterday.strftime('%b %d')} – {today.strftime('%b %d')}, {today.year}"
    )
    payload = {
    ...
    with patch.object(
        service,
        "get_date_interval",
        return_value=(today.replace(day=1), today.replace(day=28)),
    ):
...
def test_get_current_interest_no_date_match(service, caplog):
    ...
    with patch.object(
        service, "get_date_interval", return_value=(date(2000, 1, 1), date(2000, 1, 2))
    ):
        with caplog.at_level("ERROR"):
            assert service.get_current_interest(payload) == -1
...
def test_get_current_interest_entry_missing_values(service, caplog):
    ...
    payload = {
        "search_parameters": {"q": "Test"},
        "interest_over_time": {"timeline_data": [{"date": "dummy", "values": None}]},
    }
    with patch.object(
        service,
        "get_date_interval",
