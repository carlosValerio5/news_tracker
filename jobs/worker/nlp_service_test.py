...
def test_extract_keywords_returns_expected_fields(processor_service):
    with patch.object(
        processor_service,
        "process_headline",
        return_value=[("Test", 1.5), ("Item", 1.0)],
    ) as mock_proc:
...
def test_extract_keywords_raises_on_fail(processor_service):
    with patch.object(
        processor_service, "process_headline", side_effect=Exception("fail proc")
    ):
...
def test_process_headline_filters_and_sorts(service):
    ...
    def Token(lemma_, pos_, is_alpha=True):
        return types.SimpleNamespace(lemma_=lemma_, pos_=pos_, is_alpha=is_alpha)
    ...
    service.get_level_of_confidence = MagicMock(
        side_effect=lambda pos, idx: {
            ("PROPN", 0): 3.0,
            ("NOUN", 1): 2.0,
            ("ADJ", 3): 1.1,
        }[(pos, idx)]
...
def test_get_principal_keyword_no_matching_labels(service):
    fake_doc = SimpleNamespace(
        ents=[Ent("Apple", "UNKNOWN"), Ent("Banana", "ALSO_UNKNOWN")]
    )
    service._nlp = MagicMock(return_value=fake_doc)
