...
def test_create_app_jwt_with_admin_enum(monkeypatch):
    ...
    token = svc.create_app_jwt({'google_id': 'g1', 'email': 'e@example.com', 'role': Scope.ADMIN})
...
def test_create_app_jwt_with_raw_role_value(monkeypatch):
    ...
    monkeypatch.setattr(jwt_service_module.jwt, 'encode', lambda payload, s, algorithm=None: 't')
...
def test_create_app_jwt_with_invalid_role_falls_back_to_user(monkeypatch):
    ...
    svc = jwt_service_module.JWTService(secret_key='s', algorithm='HS256')
    payload = svc.create_app_jwt({'google_id': 'g3', 'email': 'u@u.com', 'role': 'something-else'})
...
def test_decode_jwt_success(monkeypatch):
    monkeypatch.setattr(jwt_service_module.jwt, 'decode', lambda token, sk, algorithms=None: {'sub': 'g1', 'email': 'e'})
