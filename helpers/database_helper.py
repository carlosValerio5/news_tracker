...
class DataBaseHelper:
    ...
    @staticmethod
    def write_batch_of_objects(
        object_type, session_factory, objects_to_write: list[object], logger: Logger
    ):
        """
        ...
        try:
            with session_factory() as session:
                stmt = (
                    insert(object_type)
                    .values(objects_to_write)
                    .on_conflict_do_nothing()
                )
                result = session.execute(stmt)
                session.commit()
                logger.info(
                    f"Keywords saved succesfully, keyword count: {result.rowcount}",
                    extra={"keywords": objects_to_write},
                )
        except Exception as e:
            logger.error("Failed to commit objects to database.", extra={"error": e})
            raise Exception(e)

    @staticmethod
    def write_batch_of_objects_and_return(
        object_type,
        session_factory,
        objects_to_write: list[object],
        logger: Logger,
        return_columns: list = None,
        conflict_index: list = None,
    ) -> list:
        """
            ...
            with session_factory() as session:
                ...
                if conflict_index:
                    ...
                    stmt = stmt.on_conflict_do_update(
                        ...
                        set_={
                            dummy_col: getattr(object_type.__table__.c, dummy_col)
                        },  # does not update
                    )
                ...
                if return_columns:
                    rows = list(result)
                    logger.info(
                        f"Keywords saved succesfully, keyword count: {len(rows)}",
                        extra={"keywords": objects_to_write},
                    )
        ...
        try:
            with session_factory() as session:
                ...
                if existing_user:
                    ...
                    raise GoogleIDMismatchException(
                        f"Google ID and email mismatch: {user_info['email']}"
                    )
            ...
            logger.exception(
                "Unexpected error checking or creating user.", extra={"error": e}
            )
            raise
