...
describe("RegisterAdmin", () => {
  ...
  it("renders and respects isActive prop", () => {
    ...
    expect(
      screen.queryByPlaceholderText("admin@example.com"),
  ...
  it("submits successfully and clears input/shows success", async () => {
    ...
    await waitFor(() =>
      expect(api.apiClient.post as jest.Mock).toHaveBeenCalled(),
    );
    expect(
      await screen.findByText(/Registration request submitted|ok/),
    ).toBeInTheDocument();
    expect(
      (screen.getByPlaceholderText("admin@example.com") as HTMLInputElement)
        .value,
    ).toBe("");
  });
  ...
  it("shows server error when response not ok", async () => {
    ...
    expect(
      await screen.findByText(/nope|Server returned 500/),
    ).toBeInTheDocument();
  });
