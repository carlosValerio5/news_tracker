...
describe('Hero component', () => {
  test('renders heading, paragraph, register button (mocked) and image', () => {
    ...
    // Heading
    expect(screen.getByRole('heading', { name: /Stay tuned/i })).toBeInTheDocument();

    // Paragraph
    expect(screen.getByText(/Register to be notified about the latest news./i)).toBeInTheDocument();
    ...
    expect(register.getAttribute('class') || register.getAttribute('className')).toContain('ml-3');

    // Image exists with the expected src and alt
    const img = screen.getByAltText('newspaper image') as HTMLImageElement;
    expect(img).toBeInTheDocument();
    expect(img.src).toContain('/src/assets/images/utsav-srestha-HeNrEdA4Zp4-unsplash.jpg');
  });
});
