...
describe("StatCard", () => {
  it("renders label and value", () => {
    render(
      <StatCard s={{ label: "Test", value_daily: 10 }} weekAndDay={false} />,
    );
  ...
  it("shows positive diff with + and green class", () => {
    render(
      <StatCard
        s={{ label: "Up", value_daily: 5, diff: 2 }}
        weekAndDay={false}
      />,
    );
  ...
  it("shows negative diff with - and red class", () => {
    render(
      <StatCard
        s={{ label: "Down", value_daily: 3, diff: -1 }}
        weekAndDay={false}
      />,
    );
  ...
  it("does not render diff when undefined", () => {
    render(
      <StatCard s={{ label: "NoDiff", value_daily: "—" }} weekAndDay={false} />,
    );
