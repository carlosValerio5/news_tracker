...
describe("LoginPage", () => {
  beforeEach(() => {
    ...
    mockTitleBar.mockImplementation(() => (
      <div data-testid="mock-titlebar">TitleBar</div>
    ));
    mockCustomGoogleLogin.mockImplementation(() => (
      <button data-testid="mock-google-login">Sign in</button>
    ));
  });
  ...
  test("renders heading, description, mocked TitleBar and Google login, and register link", () => {
    ...
    // Description
    expect(
      screen.getByText(/Welcome back! Please login with your Google account./i),
    ).toBeInTheDocument();
