...
describe('useAdminMetrics', () => {
  ...
  it('handles network errors and reports a failure', async () => {
    (api.apiClient.get as jest.Mock).mockRejectedValueOnce(new Error('network fail'));

    render(<Harness />);

    await waitFor(() => expect(screen.getByTestId('loading').textContent).toBe('false'));
    // hook sets a generic final error message
    expect(screen.getByTestId('error').textContent).toBe('Failed to load admin metrics');
  });
